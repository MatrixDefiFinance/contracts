...
// added functionnalities by the matrixdefi's devs
//
...
contract SmartChef is Ownable {
    ...
    // The block number when PILL mining ends.
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 pillReward = multiplier.mul(rewardPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
    ...
    // Stake PILL tokens to TheBushV1
    ...
    // Withdraw PILL tokens from STAKING.
